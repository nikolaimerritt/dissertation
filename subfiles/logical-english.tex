...
\end{lstlisting}
then the atomic formula \codeword{emily smith eats at cafe jaune} would be logically implied by the second clause in the listing. 
...
\\
\\
A meta atomic formula must have the keyword \codeword{that} preface the atomic formula that it describes. An example is given in Listing \ref{le:meta}.
...
\end{lstlisting}
In Listing \ref{le:meta}, the atomic formula \codeword{bob pays money to LE bank} features in the meta atomic formula \codeword{there is a requirement that _}.
